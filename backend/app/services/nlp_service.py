...
class NLPService:
    ...
    def generate_security_report(self, query: str, contexts: list, intent: str = "SUMMARIZATION", is_fallback: bool = False, requested_time: str = "알 수 없는 시간", mode: str = "summary"):
        ...
        # 의도별 맞춤 지침
        intent_instructions = {
            "사람 수": "사용자가 개체 수를 궁금해하므로, 검색된 장면들에서 발견된 인물, 차량 등의 수량을 정확히 파악하여 보고서 서두에 요약해 주세요. (예: '총 3명의 인물이 포착되었습니다.')",
            "오류 감지": "사건의 원인과 경위를 분석하는 것이 목표입니다. 왜 이런 상황이 발생했는지, 이전 장면과 어떤 연관이 있는지 논리적으로 추론하여 '주요 분석' 섹션에 상세히 기술해 주세요.",
            "행동": "이상 행동 탐지가 핵심입니다. 인물의 움직임이 왜 수상한지(예: 주변을 살핌, 담을 넘으려 함) 보안 관점에서 위험 요소를 중점적으로 분석해 주세요.",
            "정보 요약": "전반적인 상황을 시간 순서대로 요약하는 데 집중해 주세요.",
            "시간": "현재 위치와 상태를 확인하는 데 집중해 주세요."
        }
        
        specific_instruction = intent_instructions.get(intent, intent_instructions["정보 요약"])
