...
const App = () => {
  ...
  const startListening = () => {
    ...
    recognition.interimResults = false; // 중간 결과는 무시하고 최종 결과만
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'not-allowed') {
        alert("마이크 권한이 거부되었습니다. 브라우저 주소창 왼쪽의 자물쇠 아이콘을 눌러 권한을 허용해 주세요.");
      } else if (event.error === 'audio-capture') {
        alert("마이크 하드웨어를 찾을 수 없거나 다른 프로그램에서 사용 중입니다. 장치 연결 상태를 확인해 주세요.");
  ...
  const handleSearch = async (text = query) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { type: 'ai', report: aiReport, results: data.results || [], intent: data.intent_info?.intent || "OOD" }]);
    ...
    <div className="app-container">
      ...
      <main className="main-content">
                        ...
                        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', marginTop: '20px'}}>
                          {msg.results.map((res, j) => (
                            <div key={j} style={{backgroundColor: '#0b0f19', borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)'}}>
                              <img src={`http://localhost:8000${res.image_path}`} style={{width: '100%', height: '120px', objectFit: 'cover'}} alt="cctv" /><div style={{padding: '10px'}}><div style={{fontSize: '10px', color: '#3b82f6', display: 'flex', justifyContent: 'space-between'}}><span>{res.timestamp}</span><span>{(res.score * 100).toFixed(0)}%</span></div><div style={{fontSize: '11px', color: '#9ca3af', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{res.description}</div></div>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="input-area"><div className="input-container" style={{padding: '20px 40px 40px', position: 'relative'}}>
          ...
          <Plus size={20} style={{color: '#6b7280', cursor: 'pointer'}} /><input type="text" placeholder="질문을 입력하세요..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSearch()} />
