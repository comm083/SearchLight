...
- [ ] **1. 실시간 YOLO 모델 연동 파이프라인 구축**
    - CCTV RTSP 스트림이나 영상 파일로부터 프레임 단위로 YOLOv8 (또는 최신버전) 추론을 수행하는 백엔드 프로세스를 구축합니다.
...
- [ ] **4. 영상 타임스탬프와 YOLO 추론 시간 정밀 동기화**
    - 백엔드 처리 지연으로 인한 시간 오차를 보정하여, 사용자가 "방금" 이라고 검색했을 때 정확히 일치하는 프레임이 반환되도록 합니다.
