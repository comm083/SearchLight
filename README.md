...
시스템의 각 모듈(의도 분류, 검색, RAG 보고서 생성 등)이 제대로 작동하는지 확인하기 위한 테스트용 프롬프트 리스트입니다.

### 1. 의도 분류 체계 테스트 (Boundary Testing)
- **중의적 질문**: "주차장에 차가 몇 시에 들어왔어?" (`시간` vs `사람 수` 분류 확인)
- **애매한 질문**: "아까 그 사람 뭐해?" (지칭어와 `행동` 의도 연결 확인)
- **일상 대화**: "오늘 날씨 어때?" (`CHITCHAT` 분류 및 가이드 답변 확인)

### 2. 시간 파싱 및 검색 범위 테스트 (Temporal Logic)
- **상대적 시간**: "방금 전 5분 동안 무슨 일 있었어?" (현재 시간 기준 정확한 범위 계산 확인)
- **광범위 시간**: "지난주 전체 상황 요약해줘." (대량 데이터 요약 성능 확인)
- **모순된 시간**: "내일 오후에 누가 담 넘었어?" (미래 시간 예외 처리 확인)

### 3. RAG 품질 테스트 (Hallucination Check)
- **없는 데이터**: "파란색 모자 쓴 사람이 로비에서 춤추는 장면 찾아줘." (지어내지 않고 결과 없음 보고 확인)
- **정보 종합**: 결과가 여러 개일 때 모순 없이 하나의 보고서로 통합하는지 확인

### 4. 수치 및 통계 테스트 (Counting Accuracy)
- **단순 집계**: "오늘 주차장에서 감지된 차량은 총 몇 대야?" (로그 내 숫자 합산 여부 확인)

### 5. 오류 감지 및 원인 분석 테스트 (Causal Analysis)
- **인과 관계**: "왜 아까 알람이 울린 거야?" (`오류 감지` 분류 및 전후 로그 대조 분석 확인)
