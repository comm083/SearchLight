...
- [ ] **1. 데이터 증강 스케일업 (2000건 이상)**
    - `data_augmentation.py`를 통해 의도별 발화 데이터를 400건씩 총 2000건 이상 확보하여 AI의 문맥 이해도를 높이고 오분류(환각)를 방지합니다.
- [ ] **2. 크롭 이미지(Crop Image) 추출 및 벡터화**
    - YOLO가 감지한 핵심 객체 영역만 크롭하여 저장하고, 이를 CLIP 등의 비전 모델로 임베딩하여 유사 인물/차량 검색의 정확도를 높입니다.
- [ ] **3. 대량 데이터 학습을 위한 훈련 파이프라인 최적화**
    - 2000건 이상의 데이터로 KoELECTRA 파인튜닝 시 GPU 메모리 누수를 방지하고 Batch Size를 최적화하여 학습(`train.py`)을 안정적으로 수행합니다.
- [ ] **4. 검색 임계값(Threshold) 동적 튜닝 및 프롬프트 엔지니어링**
    - FAISS 검색 결과의 유사도 임계값을 높여 관련 없는 정보가 LLM에 컨텍스트로 들어가는 것을 차단하고, LLM 프롬프트를 개선하여 환각 답변을 제어합니다.
- [ ] **5. 오탐(FP) 및 미탐(FN) 데이터 자동 수집 체계 구축**
    - 사용자가 검색 결과에 '잘못된 결과'라고 피드백을 주면, 해당 데이터와 로그를 모아 주기적으로 모델을 재학습하는 Active Learning 루프를 만듭니다.
...
- [ ] **2. YOLO 감지 이벤트 DB 적재**
    - 모의 데이터 대신 실제 YOLO가 녹화 영상에서 감지한 객체(사람, 차량 등)와 이벤트를 Supabase `alerts` 및 `yolo_events` 테이블에 Insert 합니다.
- [ ] **3. 바운딩 박스(Bounding Box) 데이터 시각화 연동**
    - 프론트엔드에서 검색된 영상을 재생할 때, YOLO가 감지한 위치 정보(좌표)를 받아 화면에 Bounding Box로 오버레이합니다.
- [ ] **4. 영상 타임스탬프와 YOLO 추론 결과 정밀 동기화**
...
-**추가**
- [ ] **1. 실시간 서비스 운영**
    - 영상이 녹화되는 동안에도 영상을 분석하고 큰 문제가 발생시 바로 해당 내용을 TTS로 출력
